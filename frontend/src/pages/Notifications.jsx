  ...
  return (
    <div className="space-y-6 max-w-3xl" data-testid="notifications-page">
      ...
      <div className="space-y-2">
        {items.length === 0 && <div className="text-slate-500 text-sm">No notifications yet.</div>}
        {items.map((n) => (
          <div key={n.id} className={`bg-white border rounded-lg p-4 flex items-start gap-3 ${n.read ? "border-slate-200 opacity-70" : "border-slate-300"}`} data-testid={`notif-${n.id}`}>
            <Bell size={18} className={n.type?.includes("critical") ? "text-red-600" : "text-slate-500"} weight={n.read ? "regular" : "fill"} />
