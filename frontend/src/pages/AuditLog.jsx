    ...
    <div className="space-y-6" data-testid="audit-page">
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        <Table>
          ...
          <TableBody>
            {items.length === 0 && (
              <TableRow><TableCell colSpan={4} className="text-center text-slate-500 py-8">No audit entries yet.</TableCell></TableRow>
            )}
            {items.map((a) => (
              <TableRow key={a.id} data-testid={`audit-row-${a.id}`}>
                ...
                <TableCell className="text-xs text-slate-600 max-w-md truncate">{JSON.stringify(a.new_value)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
