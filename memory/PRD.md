...
- **Documentation pack (Feb 2026)**: New consolidated `KEYS_AND_ENV.md` (with real env values), `MIGRATION.md` (self-hosted VPS playbook), `VERCEL_DEPLOYMENT.md` (Vercel + Railway with full Emergent-removal procedure), full README rewrite, `PLAYSTORE_UPLOAD.md` (direct APK distribution + optional Play Store).
- **Light / Dark mode**: Three-state toggle (System / Light / Dark) via `ThemeProvider`. Dark palette: `bg-zinc-950` / `text-zinc-50` / `border-zinc-800`. AI buttons use `.ai-gradient` (indigo → purple → pink). **Dialog readability fix (Feb 2026)**: all form labels/inputs/selects/outline-buttons now use explicit `dark:bg-zinc-800 dark:text-zinc-50 dark:border-zinc-700` so dark-mode dialogs are fully readable.
- **Jira facility — REMOVED (Feb 2026)** per user request. Code recoverable from git history.
- **Android-ready PWA + APK build kit**: `manifest.json` with display:standalone, app shortcuts, maskable icons. `/app/android/` build kit (`twa-manifest.json` + `build.sh`) for one-command APK/AAB generation on user's laptop (container is aarch64; cannot build locally).
- **APK download facility (temporary)**: `GET /api/download/apk` + frontend gradient banner — auto-shows on Login + every page when the APK exists at `/app/android/app-release-signed.apk`.
